...
# before_filter :configure_account_update_params, only: [:update]
  ...
  def create
    binding.pry
    # these_params = params[:chef]
    @chef = Chef.new(chef_params)
    if @chef.save
      flash[:success] = "Welcome Chef! You have signed up successfully."
    else
      render :new
    # super
    end
  ...
  def chef_params
    params.require(:chef).permit(:first_name, :last_name, :email, :restaurant_name, :address, :city, :state, :zip, :phone_number, :description, :password, :password_confirmation)
  end
